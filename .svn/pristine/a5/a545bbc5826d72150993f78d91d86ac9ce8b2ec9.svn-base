 ...
 * 
 */
public class GetOrderSystemInfoHandler extends DataHandler {
	private Context mContext;
	private String server_url;
	private PdaRequest<String> orderDto;

	public GetOrderSystemInfoHandler(Context context,
			PdaRequest<String> orderDto) {
		this.mContext = context;
		this.server_url = NetWork.GET_ORDER_SYSTEM_INFO_ACTION;
	...
	protected void onNetReceiveOk(byte[] receiveBody) {
		sendResult(NetWork.GET_ORDER_SYSTEM_INFO_OK, receiveBody);
	}

	@Override
	protected void onNetReceiveError(int errorCode) {
		sendResult(NetWork.GET_ORDER_SYSTEM_INFO_ERROR, errorCode);
	}
