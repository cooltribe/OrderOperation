 ...
 * 
 */
public class SubmitOrderPhoto2Handler extends DataHandler {
	private Context mContext;
	private String server_url;
	private PdaRequest<TmDispatchDto> tmDispatchDto;

	public SubmitOrderPhoto2Handler(Context context,
			PdaRequest<TmDispatchDto> tmDispatchDto) {
		this.mContext = context;
		this.server_url = NetWork.SUBMIT_ORDER_PHOTO2_ACTION;
	...
	protected void onNetReceiveOk(byte[] receiveBody) {
		sendResult(NetWork.SUBMIT_ORDER_PHOTO2_OK, receiveBody);
	}

	@Override
	protected void onNetReceiveError(int errorCode) {
		sendResult(NetWork.SUBMIT_ORDER_PHOTO2_ERROR, errorCode);
	}
